...
class GoForward():
    ...
    def robot_parameters(self, robot):
        ...
        self.EulerAngles[0] = (math.atan2(
            2.0 * (quaty * quatz + quatx * quatw), (-x2 - y2 + z2 + w2)) * (180.0/math.pi))
        self.EulerAngles[1] = (
            math.asin(-2.0 * (quatx * quatz - quaty * quatw)) * (180.0/math.pi))
        self.EulerAngles[2] = (math.atan2(
            2.0 * (quatx * quaty + quatz * quatw), (x2 - y2 - z2 + w2)) * (180.0/math.pi))
    ...
    def navigation(self):
        ...
        while(True) & (self.new_goal == True):
            # print "again inside"
            while(self.heading == False):
                slope, line_angle, distance = self.path_calculation(
                    self.final_x, self.CurrentX, self.final_y, self.CurrentY)
                self.turn_toward_destination(line_angle, self.EulerAngles[2])
                # print distance
    ...
    def follow_slop_line(self, startx, starty):
        while(True):
            slope, line_angle, distance = self.path_calculation(
                self.final_x, startx, self.final_y, starty)
            ...
            if(self.CurrentY <= (self.CurrentX * slope) + 1) & (self.CurrentY >= (self.CurrentX * slope) - 1):
                if line_angle - self.degree_to_rad(self.EulerAngles[2] < 0.01):
                    ...
                    self.turn_toward_destination(
                        line_angle, self.EulerAngles[2])
                    distance = self.distance(
                        self.final_x, self.CurrentX, self.final_y, self.CurrentY)
                    print(distance)
                    break
